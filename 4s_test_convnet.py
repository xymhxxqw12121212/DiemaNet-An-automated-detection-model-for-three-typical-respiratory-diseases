...
print('*******************')
print("测试 mfcc+wav2vec2+tccnn_batch64")
print('*******************')
...
npz_path ='/root/autodl-tmp/feature/test'#远程服务器
lable_npz = []
mfcc_npz = []
openl_npz = []
wav2vec_npz = []
...
for i in range(len(lable_npz)):
    ...
    ls = np.load(r'/root/autodl-tmp/feature/test/{}'.format(lable_npz[i]))
    keys = ls.files
    for key in keys:
    ...
    if len(openl_npz) > 0:
        ...
        for key in keys:
            X1_test.append(ops[key])
    if len(wav2vec_npz) > 0:
        # ops = np.load(r'./feature/test/{}_wav2vec.npz'.format(ill))
...
# if len(openl_npz) >0 or len(wav2vec_npz):
#     X2_test = reshape_numpy(X2_test,transpose = False)
for i in range(0,1):
    fold_dict = dict()
    ...
    model_path = r'./modelsave_balanced_dataset/mfcc+wav2vec_tccnn.pth'
    # model_path = r'./modelsave_balanced_dataset/best_network.pth'
    if len(X2_test) > 0:
        ...
        # model = fusion_googlenet_googlenet_openl_EMA()  # 用于mfcc、openl两两组合
        # model = TCCNN_E()
