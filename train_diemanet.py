...
print(device)
print('*******************')
print("训练 mfcc+openl+diemanet_batch64")
print('*******************')
    ...
    for epoch in range(n_epoch):
        ...
        for batch_idx, dataset in enumerate(dataLoader):
            ...
            if len(dataset)== 2:
                ...
                predict,loss1 = model(data1, data2)
                ##googlenet
...
#根据不同音频长度需要修改路径
# npz_path ='/root/autodl-tmp/feature3/train'
npz_path ='/root/autodl-tmp/feature1s_6ms/train' ##生成的mfcc和openl特征维度为（167, 64)）和（10, 512）
lable_npz=[]
mfcc_npz=[]
openl_npz=[]
...
for i in range(len(lable_npz)):
    ls = np.load(r'/root/autodl-tmp/feature1s_6ms/train/{}'.format(lable_npz[i]))
    keys = ls.files
    for key in keys:
        y_train.append(ls[key])
    ill= lable_npz[i].split('_')[0]
    if len(mfcc_npz) > 0:
        ms = np.load(r'/root/autodl-tmp/feature1s_6ms/train/{}_mfcc.npz'.format(ill))
        for key in keys:
            X1_train.append(ms[key])
    if len(openl_npz) >0:
        ops = np.load(r'/root/autodl-tmp/feature1s_6ms/train/{}_openl3.npz'.format(ill))
        for key in keys:
            X2_train.append(ops[key])

##读取测试集的特征和标签
npz_path2 ='/root/autodl-tmp/feature1s_6ms/test'
lable2_npz=[]
openl2_npz=[]
mfcc2_npz=[]
...
y_test = []
for i in range(len(lable_npz)):
    ls = np.load(r'/root/autodl-tmp/feature1s_6ms/test/{}'.format(lable2_npz[i]))
    keys = ls.files
    for key in keys:
...
avauc=[]
for i in range(0,1):
    save_path1 = r'./modelsave_balanced_dataset/1s_6ms_diemanet.pth'
    save_path = r'./modelsave_balanced_dataset'  # 使用早停
    early_stopping = EarlyStopping(save_path,patience=15)
    # model = TCCNN_D()
    # model = TCCNN_E()
